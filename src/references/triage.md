...
The loop for hard bugs and performance regressions. Each phase is an atomic step in the normal vibe loop: report it in the Done format, then present the picker. Skip a phase only when you say why.

Intake is a grilling round at Diagnosis intensity (`grilling.md`): exact symptom, repro steps, expected vs actual, last known good, recent changes.
...
Before suggesting "finish": the original repro no longer reproduces, the regression test passes (or the missing seam is documented), all `[DEBUG-...]` logs are removed, throwaway harnesses are deleted, and the root cause is recorded in the Done report and proposed commit summary.
